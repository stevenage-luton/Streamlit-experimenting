...
st.write("Here's my attempt at using a dataframe, I sure hope I haven't massively fucked anything up:")

st.write(pd.DataFrame({
    'column 1': [90, 1,200,12.3, 10, 1000, 2],
    'column 2': [12, 0, 1, 1, 1, 1, 1]
}))

import matplotlib.pyplot  as plt
import numpy as np
import pandas as coolpanda
...
dataframe1 = coolpanda.DataFrame(
...
st.dataframe(dataframe1.style.highlight_min(axis=0))

dataframe2 = pd.DataFrame(
    np.random.randn(10, 20),
    columns=('col %d' % i for i in range(20)))

st.dataframe(dataframe2.style.format(precision = 2).format_index(str.upper, axis=1).relabel_index(["row 1", "row 2", "row 2", "row 2", "row 2", "row 2", "row 2", "row 2", "row 2", "row 3"], axis=0))
st.dataframe (dataframe2.style.highlight_max(color="red",axis= 1) )

st.table(dataframe2)

map_data = pd.DataFrame(
    np.random.randn(40, 2) / [50, 50] + [50.71, -3.53],
    columns=['lat', 'lon'])
